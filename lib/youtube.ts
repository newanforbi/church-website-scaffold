...
let innertubeKeyScrape: Promise<string | null> | null = null;

async function getInnertubeKey(): Promise<string> {
  if (cachedInnertubeKey) return cachedInnertubeKey;

  // Share one scrape between the concurrent channel/playlist fetches below,
  // and only memoize a key we actually scraped — caching the hardcoded
  // fallback after a transient failure would pin this process to a stale key
  // for its whole lifetime, with no retry.
  innertubeKeyScrape ??= (async () => {
    const html = await fetchText("https://www.youtube.com/");
    return html?.match(/"INNERTUBE_API_KEY":"([^"]+)"/)?.[1] ?? null;
  })().finally(() => {
    innertubeKeyScrape = null;
  });

  const scraped = await innertubeKeyScrape;
  if (scraped) cachedInnertubeKey = scraped;
  return scraped ?? FALLBACK_INNERTUBE_KEY;
}
